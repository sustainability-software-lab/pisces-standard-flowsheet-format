...
def _outlet_p_unit(class_name, outlet_P=None, bases=(), **attrs):
    """Build a fake unit whose CLASS NAME matters (the exporter's outlet-P
    substitution is gated on the unit being a Pump or MolecularSieve, checked
    by class name across the MRO so model subclasses qualify too). `outlet_P`,
    when given, becomes outs[0].P."""
    cls = type(class_name, bases, {})
    unit = cls()
    if outlet_P is not None:
        unit.outs = [types.SimpleNamespace(P=outlet_P)]
    unit.__dict__.update(attrs)
    return unit


class TestGetDesignInputSpecsOutletPSubstitution(unittest.TestCase):
    """Pump/MolecularSieve spec substitution (0.1.3+). In biosteam, both
    classes default to P=None, in which case the simulated outlet keeps the
    inlet's pressure (Pump._run: `if self.P: s_out.P = self.P`;
    MolecularSieve._run: `if P is None: P = self.ins[0].P` then sets every
    outlet) -- so a bare {"P": null} spec is uninformative, and the actual
    applied outlet pressure is unit.outs[0].P. The exporter substitutes that
    value under the existing "P" key for exactly these two classes (no new SFF
    key, no dP_design; deliberate format decision 2026-08-26, revised same
    day from an earlier dP_design-based shape). Gated by the
    substitute_outlet_P flag so pre-0.1.3 exporters stay byte-stable."""

    def test_pump_with_none_P_substitutes_outlet_P_when_enabled(self):
        """A Pump with P=None exports outs[0].P under the 'P' key when the
        0.1.3+ flag is on; dP_design is NOT consulted (set here to a
        sentinel that must not appear)."""
        unit = _outlet_p_unit("Pump", outlet_P=1.5e5, P=None, dP_design=999999.0)
        self.assertEqual(
            _export.get_design_input_specs(unit, substitute_outlet_P=True),
            {"P": 1.5e5})

    def test_molecular_sieve_with_none_P_substitutes_outlet_P(self):
        """A MolecularSieve with P=None exports outs[0].P under 'P' -- in
        biosteam this class even hardcodes self.P = None at _init, so its
        spec is null on every export without this substitution."""
        unit = _outlet_p_unit("MolecularSieve", outlet_P=2.02e5, P=None)
        self.assertEqual(
            _export.get_design_input_specs(unit, substitute_outlet_P=True),
            {"P": 2.02e5})

    def test_pump_subclass_qualifies_via_mro(self):
        """A model-defined Pump subclass is still a Pump: the class check
        walks the MRO by name, so the substitution applies."""
        pump_cls = type("Pump", (), {})
        unit = _outlet_p_unit("CustomModelPump", outlet_P=1.1e5,
                              bases=(pump_cls,), P=None)
        self.assertEqual(
            _export.get_design_input_specs(unit, substitute_outlet_P=True),
            {"P": 1.1e5})
    ...
    def test_pump_with_none_P_keeps_null_P_by_default(self):
        ...
        unit = _outlet_p_unit("Pump", outlet_P=1.5e5, P=None)
        self.assertEqual(_export.get_design_input_specs(unit), {"P": None})

    def test_pump_with_truthy_P_is_not_substituted(self):
        """A pump with an explicit outlet-pressure spec (P=2e6) keeps it (for
        a simulated Pump, unit.P IS outs[0].P in that case anyway)."""
        unit = _outlet_p_unit("Pump", outlet_P=2e6, P=2e6)
        self.assertEqual(
            _export.get_design_input_specs(unit, substitute_outlet_P=True),
            {"P": 2e6})

    def test_pump_with_zero_P_is_substituted(self):
        """Pump._run treats P=0 exactly like P=None (truthiness), so the
        exporter mirrors that: a zero P is replaced by the true outs[0].P."""
        unit = _outlet_p_unit("Pump", outlet_P=1.5e5, P=0.0)
        self.assertEqual(
            _export.get_design_input_specs(unit, substitute_outlet_P=True),
            {"P": 1.5e5})

    def test_other_unit_class_with_none_P_is_unchanged(self):
        """A unit of any other class with P=None keeps the plain null spec
        even with the flag on -- the substitution is Pump/MolecularSieve
        only."""
        unit = _outlet_p_unit("Flash", outlet_P=3e5, P=None)
        self.assertEqual(
            _export.get_design_input_specs(unit, substitute_outlet_P=True),
            {"P": None})
